...
class PerfectTradingBot:
    ...
    def _create_perfect_signal(self):
        ...
        try:
            ...
            # 완벽 파워 계산
            perfect_power = market_boost * time_boost * combo_boost * cycle_boost * self.signal_multiplier
            ...
            return {
                ...
                'perfect_power': random.uniform(1.5, 2.0),
                'base_strength': 0.8,
                'timestamp': datetime.now()
            }
    ...
    def _perfect_risk_check(self, price, direction):
        """완벽 리스크 체크"""
        try:
            # RiskManager에 check_risk 메서드가 없을 수 있으므로 자체 구현
            ...
            logger.warning(f"리스크 체크 오류: {e}, 기본 허용")
            return {
                'allowed': True,
                'reason': '기본 허용'
            }
